...
var GeminiService = class {
  ...
  async validateApi(modelName) {
    const genAI = this.getGenAI();
    if (!genAI) {
      new import_obsidian.Notice("Gemini API key is not set.");
      return false;
    }
    try {
  ...
  async transcribeAudio(audioPath, prompt, modelName) {
    const genAI = this.getGenAI();
    if (!genAI) {
...
var Tella = class extends import_obsidian.Plugin {
  ...
  async transcribeAudioInNote() {
    ...
    if (!activeView) {
      new import_obsidian.Notice("No active note to transcribe.");
      return;
    }
...
`;
        const newEndPos = editor.offsetToPos(editor.posToOffset(startPos) + placeholderBlock.length);
        editor.replaceRange(errorBlock, startPos, newEndPos);
        new import_obsidian.Notice(`Could not find the file: ${fileName}`);
      }
    }
    const promises = jobs.map(async (job) => {
      const updateCallout = (blockToInsert) => {
        ...
        if (placeholderIndex !== -1) {
          ...
          new import_obsidian.Notice(`Could not update transcription for ${job.fileName}.`);
        }
      };
      try {
...
`;
    ...
    new import_obsidian.Notice("All transcriptions complete!");
  }
};
var TellaSettingTab = class extends import_obsidian.PluginSettingTab {
